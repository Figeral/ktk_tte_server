      ...
      <div class="row">
        <div class="col-md-8">
          <div class="btn-group">
            <?php if ($user_role == 1) { ?>
              <div class="btn-group">
                <a title="Create Task" href="task-create.php"> <button class=" btn btn-primary btn-menu">Create New Task</button></a>
              </div>
            <?php } ?>
          ...
          <tbody>
            ...
            if ($user_role == 1) {
              $sql = "SELECT a.*, b.fullname 
                        FROM task a
                        INNER JOIN tbl_admin b ON( a.user_id = b.user_id )
                        ORDER BY a.id DESC";
            } else {
              $sql = "SELECT a.*, b.fullname 
                  FROM task a
            ...
            ?>
              <tr>
                ...
                <td>
                  <?php if ($row['status'] == 1) {
                    echo " <span class='label label-warning' ><i class=' glyphicon glyphicon-refresh'></i> Pending</span>";
                  } elseif ($row['status'] == 2) {
                    echo " <span class='label label-success'><i class='glyphicon glyphicon-ok'></i> Active </span>";
                  } else {
                    echo "<span class='label label-default'><i class='glyphicon glyphicon-remove'></i> Archived </span>";
                  } ?>

                </td>

                <td><a title="Update Task" href="edit-task.php?task_id=<?php echo $row['id']; ?>" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit"></span></a>&nbsp;&nbsp;
                  <a title="View" href="task-details.php?task_id=<?php echo $row['id']; ?>" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-folder-open"></span></a>&nbsp;&nbsp;
                  <?php if ($user_role == 1) { ?>
                    <a class="btn btn-danger btn-sm" title="Delete" href="?delete_task=delete_task&task_id=<?php echo $row['id']; ?>" onclick=" return check_delete();"><span class="glyphicon glyphicon-trash"></span></a>
                </td>
              <?php } ?>
              </tr>
            <?php } ?>
