...
class Admin_Class
{
	...
	public function admin_login_check($data)
	{
		...
		try {
			$stmt = $this->db->prepare("SELECT * FROM tbl_admin WHERE username=:uname AND password=:upass ");
			...
			if ($stmt->rowCount() != 0) {
				...
				$message = 'Username  ' . $username . '  password  ' . $upass . ' number of row   {rowNumber}';
				$message = str_replace("{rowNumber}", $stmt->rowCount(), $message);
				return $message;
			}
